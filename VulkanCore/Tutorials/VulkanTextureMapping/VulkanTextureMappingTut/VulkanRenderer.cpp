...
void VulkanRenderer::CreateGraphicsPipeline() {
	auto vertShaderCode = readFile("../VulkanTextureMappingShaders/vert.spv");
	auto fragShaderCode = readFile("../VulkanTextureMappingShaders/frag.spv");
...
void VulkanRenderer::TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout,
	...
	// Determine the source and destination pipeline stages based on the layouts
	VkPipelineStageFlags sourceStage;
	VkPipelineStageFlags destinationStage;

	if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED && newLayout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
		barrier.srcAccessMask = 0; // No need to wait on anything
